...
public enum Status {
    ...
    ACCOUNT_NULL(4006, "该用户不存在，请注册后登录", false),
    PASSWORD_INCORRECT(4007, "用户名或密码错误", false),
    ACCOUNT_LOCKED(4008, "账户已被锁定", false),
    ACCOUNT_REPEAT(4009, "该昵称已被占用", false),
    GET_CURRENT_USER_FAILURE(40010, "当前用户获取失败", false),
    //业务
    ARTICLE_LIST_NULL(5003, "文章列表为空", false),
    PUBLISH_FAILURE(5004, "文章发表失败,请登录后尝试", false),
    ARTICLE_NULL(5005, "该文章不存在", false),
    ARTICLE_LIST_FAILURE(5006, "获取文章列表失败", false);
